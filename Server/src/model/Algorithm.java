 ...
 * Any message longer than that will be automatically shortened to 190 characters if "Encrypt" radio button is selected when pressing "Process" button.
 * There is no limit when "Decrypt" radio button is selected, each encrypted message will be longer than before mentioned limit.
    ...
    public final String encrypt(String data, String publicKey) throws AlgorithmException {
        try {
            ...
            for(byte[] elem : resultList)
                result.append(Base64.getEncoder().encodeToString(elem));
